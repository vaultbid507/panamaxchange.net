(()=>{
   ...
   *
   * Process:
   * 1. Reuse the exact Supabase client initialized by admin-auth.js.
   * 2. Never create a second client with a duplicated or stale API key.
   * 3. Require an authenticated administrator without replacing the shared session.
   * 4. Load products and auctions through the protected Admin RPCs.
   * 5. Render the auction table with search and status filtering.
   * 6. Open Create and Edit forms in centered Product-style modal popouts.
   * 7. Validate all auction values before mutation.
  ...
  const wait=promise=>Promise.race([
    promise,
    new Promise((_,reject)=>setTimeout(()=>reject(new Error('Request timed out. Check your Admin session and Supabase connection.')),12000))
  ]);

  /** Resolve the single Supabase client created by admin-auth.js. */
  function getClient(){
    if(db) return db;
    db=window.PanamaAdminAuth?.client||null;
    if(!db) throw new Error('Admin authentication is still initializing. Please wait a moment and refresh.');
    return db;
  }
  ...
  /** Load products for both create and edit selectors. */
  ...
  /** Open the Edit Auction modal and populate the selected record. */
  ...
  /** Build and validate an auction RPC payload. */
  ...
  /** Bind row actions after each table render. */
  function bindRows(){
  ...
  /** Initialize Auction Management after the shared Admin auth initializer completes. */
  async function start(){
    try{
      ...
      bindModalDismiss();
    }catch(error){$('auctionRows').innerHTML=`<tr><td colspan="7" class="error">Unable to load auctions: ${esc(error.message||error)}</td></tr>`;}
  }
